...
/*   Updated: 2023/10/23 18:14:05 by rnaito           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_cylinder(t_scene *scene, const char *const line)
{
	...
	circle_center = get_circle_center(
			tube->center, tube->direction_vec, tube->height / 2);
	top = set_circle(circle_center, tube->diameter,
			tube->direction_vec, tube->color);
	scene->objects[scene->objects_num] = create_object(
			(void *)top, hit_circle, get_normal_vector_for_circle,
			get_pixel_color_for_circle);
	scene->objects_num++;
	resize_objects_array(&scene->objects, scene->objects_num);
	circle_center = get_circle_center(
			tube->center, tube->direction_vec, tube->height / 2 * -1);
	bottom = set_circle(circle_center, tube->diameter,
			tube->direction_vec, tube->color);
	scene->objects[scene->objects_num] = create_object(
			(void *)bottom, hit_circle, get_normal_vector_for_circle,
			get_pixel_color_for_circle);
	scene->objects_num++;
}
